...
async def ask_nora(user_id, text):
    key = str(user_id)
    ...
    messages = [{"role": "user", "content": NORA_PROMPT}] + conversation_history[key]
...
async def remind_checkin():
    ...
    if channel:
        await channel.send("今天的日結呢？三餐加運動，還沒看到。別想消失。")
...
async def on_message(message):
    ...
    if any(k in message.content for k in meal_keywords):
        async with message.channel.typing():
            reply = await ask_nora(message.author.id, message.content)
